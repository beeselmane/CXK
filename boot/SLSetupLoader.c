...
SLEFIABI void SLStartSetup(SLFileHandle setupFile, CXKBootOptions *options)
{
    ...
    SLPrintString(SLS("Reading ELF Header...\r\n"));
    ...
    if (!options->memoryMap.key) return;
        ...
        options->devOptions.serialPort0 = SLInitSerial((UInt64)options->devOptions.serialPort0);
        CXKBasicSerialSetupLineControl(options->devOptions.serialPort0, config->devOptions.serial.wordLength, config->devOptions.serial.stopBits, config->devOptions.serial.parity);
        CXKBasicSerialSetBaudDivisor(options->devOptions.serialPort0, config->devOptions.serial.baudDivisor);
