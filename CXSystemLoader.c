...
void CLSerialWrite(const char *s)
{
    while (s) CXKBasicSerialWriteCharacter(gSerialPort0, *s++, true);
}
...
void noSerialCheck()
{
    ...
    CXKWriteByte(&port->scratchSpace, testValue);
    UInt8 portValue = CXKReadByte(&port->scratchSpace);
...
void CXSerialSetup()
{
    ...
    CXKBasicSerialSetBaudRate(gSerialPort0, 57600);

#if 0
    UInt8 interruptsEnabled = CXKReadByte(&gSerialPort0->byte1.interruptsEnabled);
    UInt8 interruptInfo = CXKReadByte(&gSerialPort0->byte2.interruptInfo);
    UInt8 lineControl = CXKReadByte(&gSerialPort0->lineControl);
    UInt8 modemControl = CXKReadByte(&gSerialPort0->modemControl);
    UInt8 lineStatus = CXKReadByte(&gSerialPort0->lineStatus);
    UInt8 modemStatus = CXKReadByte(&gSerialPort0->modemStatus);
    UInt8 scratchSpace = CXKReadByte(&gSerialPort0->scratchSpace);

    CXKWriteByte(&gSerialPort0->lineControl, lineControl | (1 << kCXKBasicSerialDivisorExposeShift));
    UInt8 divisorLSB = CXKReadByte(&gSerialPort0->byte0.divisorLSB);
    UInt8 divisorMSB = CXKReadByte(&gSerialPort0->byte1.divisorMSB);
    CXKWriteByte(&gSerialPort0->lineControl, lineControl);
...
CXStatus CXEFI CXSystemLoaderMain(input CXHandle imageHandle, input CXSystemTable *systemTable)
{
    ...
    gSerialPort0 = CXKBasicSerialIntializeAtAddress((OSAddress)kCXKBasicSerialDefaultOffset);
